...
public class SCardReaderList: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    private func getUnpoweredSlotsCount() -> Int {
        ...
        os_log("Non powered slot count: %d", log: OSLog.libLog, type: .debug, count)
        return count
    }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        if error != nil {
            _ = self.generateError(code: error!._code, message: error!.localizedDescription, trigger: true)
        } else {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        os_log("Characteristic ID: %s", log: OSLog.libLog, type: .debug, characteristic.uuid.uuidString)
        if error != nil {
            let _error = generateError(code: error!._code, message: error!.localizedDescription)
            switch lastCommand {
            case .control:
                callOnControlDidResponseWithError(_error)
            case .cardConnect:
    ...
    public static func create(peripheral: CBPeripheral, centralManager: CBCentralManager, advertisingServices: [CBUUID], delegate: SCardReaderListDelegate, secureConnectionParameters: SecureConnectionParameters? = nil)  {
        ...
        if detectedDeviceType == .Unknown {
            ...
            os_log("Error code: %d, Message: %s", log: OSLog.libLog, type: .error, errorCode, errorMessage)
